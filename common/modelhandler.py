...
class CModelHandler(object):
    def __init__(self, model, checkpoint_path, args, **kwargs):
        ...
        self.epochs_to_save = (
            np.concatenate((range(1, 10, 3), range(10, 100, 10), range(100, 10000, 50)))
            if kwargs.get("epochs_to_save") is None
            else kwargs.get("epochs_to_save")
        )
    ...
    def load_pretrained(self):
        ...
        if self.load_mode == "last":
            ...
            self.model.load_state_dict(
                pretrained_dict["state_dict"].module.state_dict()
            )
