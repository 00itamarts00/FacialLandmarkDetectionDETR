...
from utils.ptsutils import decode_preds_heatmaps
...
def train_epoch(train_loader, model, criteria, optimizer, epoch, writer_dict, **kwargs):
    ...
    for batch_idx, item in enumerate(train_loader):
        ...
        target_dict = {
            "labels": [torch.range(start=0, end=tpts.shape[1] - 1) for i in range(bs)],
            "coords": tpts,
            "heatmap_bb": heatmaps,
        ...
        if batch_idx % log_interval == 0:
            ...
            msg = (
                f"Epoch: [{str(epoch).zfill(3)}][{str(batch_idx).zfill(3)}/{len(train_loader)}]\t"
    ...
    msg = (
        f"Train Epoch {epoch} | time: {batch_time.avg:.4f} | loss: {losses.avg:.4f} | NME: {nme:.4f}"
...
def validate_epoch(val_loader, model, criteria, epoch, writer_dict, **kwargs):
    ...
    with torch.no_grad():
        for i, item in enumerate(val_loader):
            ...
            target_dict = {
                "labels": [
                    torch.range(start=0, end=tpts.shape[1] - 1) for i in range(bs)
                ],
                "coords": tpts,
                "heatmap_bb": heatmaps,
                "weighted_loss_mask_awing": weighted_loss_mask_awing,
            ...
            output, preds = inference(model, input_batch=input_, **kwargs)
            loss_dict, lossv = get_loss(
                criteria, output, target_dict=target_dict, **kwargs
            )
    ...
    msg = (
        f"Test Epoch {epoch}  | time: {batch_time.avg:.4f} | loss:{losses.avg:.4f}  | NME: {nme:.4f}"
        f" | AUC08: {auc08_epoch:.3f} | FR08: {failure_008_rate:.3f}"
    )
    logger.info(msg)
