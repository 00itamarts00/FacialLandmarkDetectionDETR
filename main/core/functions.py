...
def train_epoch(train_loader, model, criteria, optimizer, scheduler, epoch, logger_cml, **kwargs):
    ...
    for batch_idx, item in enumerate(train_loader):
        ...
        input_, tpts, scale = item['img'], item['tpts'].cuda(), item['sfactor'].cuda()
        # compute the output
        bs = tpts.shape[0]
