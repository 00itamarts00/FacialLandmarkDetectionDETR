...
class LDMTrain(object):
    def __init__(self, params, single_image_train=False):
        ...
        self.params = params
        self.workset_path = os.path.join(
            self.ds["dataset_dir"], self.ds["workset_name"]
        )
    ...
    def init_wandb_logger(self):
        ...
        wandb.init(
            project="detr_landmark_detection",
            # group='nothing',
            config=self.pr,
        )
    ...
    def create_dataloaders(self, datasets=None):
        ...
        dftrain = df.sample(
            frac=trainset_partition, random_state=partition_seed
        ...
        valid_loader = data.DataLoader(
            validset, batch_size=batch_size, shuffle=False, **kwargs
        )
    ...
    def load_model(self):
        ...
        if self.tr["model"] == "HRNET":
            ...
            hrnet_config._C.defrost()
            hrnet_config._C.MODEL.INIT_WEIGHTS = False
            hrnet_config._C.freeze()
            model = get_face_alignment_net(hrnet_config._C)
            # state_dict = torch.load(hrnet_config._C.MODEL.PRETRAINED)
    ...
    def train(self):
        ...
        for epoch in range(self.last_epoch + 1, epochs):
            ...
            FileHandler.save_dict_to_pkl(
                self.writer["log"], os.path.join(self.paths.stats, "meta.pkl")
            )
            ...
            if is_best or epoch % 20 == 0:
                ...
                final_model_state_file = os.path.join(
                    self.paths.checkpoint, "final_state.pth"
                )
