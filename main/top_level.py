...
class TopLevel(object):
    ...
    def setup_workspace(self):
        ...
        FileHandler.save_dict_as_yaml(
            self.params, os.path.join(ex_workspace_path, "params.yaml")
        )
    ...
    def single_batch_train(self):
        ...
        override_params = {
            "train": {"epochs": 150, "batch_size": 32, "cuda": {"use": True}},
    ...
    def train(self):
        ...
        lmd_train = LDMTrain(
            params=self.params,
            single_image_train=self.params["experiment"]["single_image_train"],
        )
        lmd_train.train()
    ...
    def evaluate_model(self):
        override_params = {
            "experiment": {
                "pretrained": {"use_pretrained": False, "timestamp": "test"}
            }
        }
        self.params = self.override_params_dict(dict_override=override_params)
    ...
    def run_experiment(self):
        self.train()
        override_params = {"experiment": {"pretrained": {"timestamp": g.TIMESTAMP}}}
        self.params = self.override_params_dict(dict_override=override_params)
        self.evaluate_model()
